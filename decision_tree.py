...
class DecisionTreeAlgorithm(Algorithm):
    ...
    def load_model(self, filepath='pacman_decision_tree.pkl'):
        """Load a trained model from disk"""
        try:
            if os.path.exists(filepath):
                with open(filepath, 'rb') as f:
                    self.classifier = pickle.load(f)
                print("Karar ağacı modeli yüklendi.")
                return True
        except Exception as e:
            print(f"Model yükleme hatası: {e}")
        return False
        
    def save_model(self, filepath='pacman_decision_tree.pkl'):
        """Save the trained model to disk"""
        if self.classifier:
            try:
                with open(filepath, 'wb') as f:
                    pickle.dump(self.classifier, f)
                print("Karar ağacı modeli kaydedildi.")
                return True
            except Exception as e:
    ...
    def save_training_data(self, training_data, filepath='pacman_training_data.csv'):
        ...
        try:
            with open(filepath, 'w') as f:
                ...
                for features, action in training_data:
                    ...
                    f.write(line + '\n')
            print(f"Eğitim verileri {filepath} dosyasına kaydedildi.")
            return True
        except Exception as e:
    ...
    def export_tree_visualization(self, filepath='pacman_decision_tree.dot'):
        ...
        if self.classifier:
            try:
                ...
                print(f"Karar ağacı görseli {filepath} dosyasına aktarıldı.")
                return True
            except Exception as e:
